...
// 3D models later. Pure Three.js, runs in the browser; wraps to iOS/Android via Capacitor.

const LANES = [-2.2, 0, 2.2];

export interface RunnerCallbacks {
  onUpdate: (distance: number, gems: number) => void;
...
export function createRunner(parent: HTMLDivElement, cb: RunnerCallbacks): RunnerHandle {
  ...
  // Lane stripes for a sense of speed.
  ...
  // Player ("dino": body + head, placeholder).
  ...
  function loop() {
    ...
    speed = Math.min(speed + dt * 0.6, 32);
    distance += speed * dt;
    const dz = speed * dt;
    ...
    // Spawn rows at a steady spacing (tightens slightly as speed rises).
    ...
    // Collisions + collection + recycling.
    for (let i = obstacles.length - 1; i >= 0; i--) {
      ...
      if (Math.abs(o.mesh.position.z) < 0.8 && o.lane === laneIndex) {
        if (o.type === 'low' && player.position.y < 1.0) endRun();
        else if (o.type === 'high' && !sliding) endRun();
      }
    }
    for (let i = gems.length - 1; i >= 0; i--) {
      ...
      if (Math.abs(g.mesh.position.z) < 0.8 && g.lane === laneIndex) {
        scene.remove(g.mesh);
        gems.splice(i, 1);
        gemCount += 1;
      }
    ...
    if (clock.elapsedTime - lastHud > 0.12) {
      cb.onUpdate(Math.floor(distance), gemCount);
